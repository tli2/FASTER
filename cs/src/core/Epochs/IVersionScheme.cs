...
public interface IVersionScheme
{
    ...
    VersionSchemeState CurrentState();
    ...
    /// <summary>
    /// Start executing the given state machine. May block forever if called under protection. Instead, use
    /// TryExecuteStateMachine if in this situation and relinquish protection between retries.
    /// </summary>
    ...
    /// or cannot be started because the version has advanced past the target version specified (FAIL)
    /// </returns>
    public StateMachineExecutionStatus TryAdvanceVersionWithCriticalSection(Action<long, long> criticalSection,
    ...
    /// <param name="spin">whether to spin wait until version transition is complete</param>
    /// <returns> whether the state machine can be executed. If false, EPVS has advanced version past the target version specified </returns>
    public bool AdvanceVersionWithCriticalSection(Action<long, long> criticalSection, long targetVersion = -1,
        bool spin = false);
}
